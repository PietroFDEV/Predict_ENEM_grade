...
# Normalize numerical features in the new data
new_X_scaled = scaler.transform(new_data)

# Make predictions using the ensemble model
new_resnet_preds = resnet_model.predict(new_X_scaled).flatten()
new_tree_preds = tree_model.predict(new_X_scaled)
new_rf_preds = rf_model.predict(new_X_scaled)
new_ensemble_preds = 0.5 * new_resnet_preds + 0.3 * new_tree_preds + 0.2 * new_rf_preds

# Add predictions as a new column in the new data
new_data['NU_NOTA_REDACAO'] = new_ensemble_preds
