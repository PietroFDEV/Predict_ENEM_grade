...
from tensorflow.keras.layers import Input, Dense, Dropout, BatchNormalization, LeakyReLU, Add, Layer
...
def build_resnet(input_shape):
    ...
    residual = Dense(256, activation=None)(residual)  # Adjust residual dimensions
    x = BatchNormalization()(x)
    x = LeakyReLU()(x)
    x = Dropout(0.3)(x)
    x = Dense(256, activation=None)(x)
...
# Decision Tree Model
tree_model = GradientBoostingRegressor(n_estimators=500, learning_rate=0.01, max_depth=6, random_state=42)
...
stacked_model = StackingRegressor(
    estimators=[
        ('resnet', GradientBoostingRegressor(n_estimators=500, learning_rate=0.01, max_depth=6, random_state=42)),
        ('tree', RandomForestRegressor(n_estimators=500, random_state=42))
    ],
